...
// MAGIC %sql
// MAGIC CREATE DATABASE IF NOT EXISTS dbacademy_pramod_dm_lntinfotech_com_db;
...
val DF = spark.read.format("delta").load("dbfs:/user/pramod.dm@lntinfotech.com/dbacademy/developer-foundations-capstone/batch_orders_dirty.delta")
DF.createOrReplaceTempView("batched_orders")
spark.sql("CACHE TABLE batched_orders") 
...
// MAGIC  %fs ls dbfs:/user/hive/warehouse/dbacademy_pramod_dm_lntinfotech_com_db.db/
...
var df = DF.withColumn("_error_ssn_format", $"sales_rep_ssn".contains('-'))
//DF.withColumn("_error_ssn_format",
    //when(col("sales_rep_ssn").contains("-"),lit("true").cast(BooleanType))
      //.otherwise(lit("false").cast(BooleanType)))
    //df=df.withColumn("sales_rep_ssn",regexp_replace($"sales_rep_ssn","-",""))   

import org.apache.spark.sql.types._

// Converting to Long and Int
var salesrepdf=df.withColumn("sales_rep_ssn",col("sales_rep_ssn").cast("long"))
    salesrepdf= salesrepdf.withColumn("sales_rep_zip",col("sales_rep_zip").cast("int"))


 salesrepdf= salesrepdf.drop("submitted_at", "order_id","customer_id", "shipping_address_attention", "shipping_address_address", "shipping_address_city", "shipping_address_state", "shipping_address_zip","product_id", "product_quantity","product_sold_price")    

salesrepdf=salesrepdf.dropDuplicates("sales_rep_id","sales_rep_ssn","sales_rep_first_name","sales_rep_last_name","sales_rep_address","sales_rep_city","sales_rep_state","sales_rep_zip","_error_ssn_format")

//spark.sql("drop table sales_reps")
//salesrepdf.write.format("delta").saveAsTable("sales_reps")

//spark.sql("drop table sales_reps")

//spark.sql("drop table sales_rep_scd")
salesrepdf.write.format("delta").mode("overwrite").saveAsTable("sales_reps")

salesrepdf.printSchema()
display( salesrepdf)
...
df_new = df_new.withColumn("_error_ssn_format", $"sales_rep_ssn".contains('-'))

     .withColumn("sales_rep_ssn",$"sales_rep_ssn".cast("long"))

       .withColumn("sales_rep_zip",$"sales_rep_zip".cast("integer"))

       .drop("submitted_at","order_id","customer_id","shipping_address_attention","shipping_address_address","shipping_address_city",

            "shipping_address_state","shipping_address_zip","product_id","product_quantity","product_sold_price")

  df_new = df_new.dropDuplicates("sales_rep_id","sales_rep_ssn","sales_rep_first_name","sales_rep_last_name","sales_rep_address","sales_rep_city","sales_rep_state","sales_rep_zip","_error_ssn_format")


display(df_new)

df_new.write.format("delta").mode("overwrite").saveAsTable("sales_reps")*/
...
val batch_source_path="dbfs:/user/pramod.dm@lntinfotech.com/dbacademy/developer-foundations-capstone/batch_orders_dirty.delta"
var df3=spark.read.format("delta").load(batch_source_path)
var df_temp = df3


df_temp = df_temp.withColumn("submitted_at", )
                 .withColumn("shipping_address_zip",$"shipping_address_zip".cast("integer"))
                 .drop("sales_rep_ssn","sales_rep_first_name","sales_rep_last_name","sales_rep_address","sales_rep_city","sales_rep_state",
                      "sales_rep_zip","product_id","product_quantity","product_sold_price")
...
val batch_source_path="dbfs:/user/pramod.dm@lntinfotech.com/dbacademy/developer-foundations-capstone/batch_orders_dirty.delta"
var df=spark.read.format("delta").load(batch_source_path)

var productDF=df.drop("submitted_at","customer_id","sales_rep_id","sales_rep_ssn","sales_rep_first_name","sales_rep_last_name",
                  "sales_rep_address","sales_rep_city","sales_rep_state","sales_rep_zip","shipping_address_attention",
                  "shipping_address_address","shipping_address_city","shipping_address_state","shipping_address_zip")

productDF=productDF.withColumn("product_quantity",col("product_quantity").cast("int"))
 productDF = productDF.withColumn("product_sold_price",col("product_sold_price").cast("decimal(10,2)"))


 productDF.write.format("delta").mode("overwrite").saveAsTable("line_items")
...
ordersDF=ordersDF.withColumn("submitted_at",col("submitted_at").cast("Timestamp"))
 ordersDF = ordersDF.withColumn("shipping_address_zip",col("shipping_address_zip").cast("int"))
ordersDF
...
spark.sql("drop table orders")
ordersDF.write.format("delta").partitionBy("submitted_yyyy_mm").saveAsTable("orders")
